...
所以 W7 输出的 STAR 草稿不是“最终背诵稿”，而是一个结构化讲法：
...
- `star_answers`：STAR 回答草稿。
- `knowledge_points`：技术复习点。
- `mock_score`：模拟面试准备分。
- `evidence_warnings`：真实性提醒。
...
```text
请结合你的项目经历，说明你如何满足“FastAPI”这个要求？
```
...
`_star_answers()` 会从简历项目和经历生成 STAR 结构。

例如项目 `CareerPilot`：

- S：项目背景；
- T：要证明哪些岗位能力；
- A：按需求理解、方案设计、核心实现、测试验证来讲；
- R：只讲证据能支撑的产出。
...
4. 调用 `InterviewCoachAgent().create_pack(...)`。
5. 完成 step。
...
```text
实现 InterviewCoachAgent，基于结构化简历、JD、匹配缺口和证据锁定改写草稿，生成面试题预测、项目追问、STAR 回答草稿、技术复习清单与模拟准备分，并通过 run trace/checkpoint 保留生成过程。
```
